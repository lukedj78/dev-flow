...
3. **Triage from `agentHandoff.actionables`, not from the human output.** Every actionable carries a
   `disposition`, a `confidence`, and the `file:line` it was found at. Act on the pair, never on the score:
