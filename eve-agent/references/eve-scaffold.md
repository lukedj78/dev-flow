...
* **Per-session token budget (cost control).** Set `limits` in `agent/agent.ts` to cap spend and
  ...
  `providerOptions.anthropic.thinkingBudget`. ⚠️ **`[VERIFY]` these against the AI SDK / AI Gateway provider docs, not against eve** — `providerOptions` is passed through to the provider, so these field names appear nowhere in `node_modules/eve/docs/` (checked on 0.38.3) and looking for them there will wrongly suggest they are stale.
