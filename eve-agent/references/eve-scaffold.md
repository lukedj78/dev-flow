...
* **Evaluation models (AI Gateway, AI SDK only).** For classification, routing, rubric scoring or verification *inside a tool*, an evaluation model answers typed questions about a `state` (string, object or array) instead of generating text: `import { experimental_evaluate as evaluate } from "ai"` (**AI SDK ≥ 7.0.105**), `model: "typesafe-ai/jev"` (or `gateway.evaluationModel("typesafe-ai/jev")` with the Gateway provider instance), `questions` of type **`boolean`** (→ `probability`, optional `criteria: { true, false }`), **`choice`** (`criteria` = option → description; → `choice` + `probabilities`) or **`score`** (`criteria` = ≥ 2 labels lowest → highest; → interpolated `score` + per-rung `probabilities`). Several questions share one request; `result.usage` reports tokens and billing is per token. **Three ways in, all billed through the Gateway and all visible in its usage and observability** (changelog 2026-09-21): the **AI SDK** (`experimental_evaluate`, the path this skill uses); an existing **TypeSafe client** pointed at the Gateway — `new TypeSafeClient({ apiKey: process.env.AI_GATEWAY_API_KEY, baseURL: "https://ai-gateway.vercel.sh/typesafe" })`, whose `systemOne()` calls and `noul` questions stay unchanged; and a native **HTTP API**, `POST https://ai-gateway.vercel.sh/v1/evaluate` with `{ model, state, questions }` and a bearer key — the way a Python or Go service calls Jev without an AI SDK. **Not available** through the OpenAI-, Anthropic- or Cohere-compatible chat endpoints. Supports `providerOptions: { gateway: { zeroDataRetention: true } }`. Vendor claim, not ours: *"up to 193.6x faster and 444.6x cheaper than LLMs"* on TypeSafe's own workflow evaluations — measure on yours. Refs: <https://vercel.com/docs/ai-gateway/modalities/evaluation> · <https://vercel.com/changelog/typesafe-ai-jev-now-available-on-ai-gateway>. The eve-side use of the same API, `auto` (renamed from `autoModel` in eve 0.60.0, now imported from `eve/models`), is in `eve-concepts.md` §Agent config; the patterns for using it inside an agent — approval policy, tool guardrail, evals, triage, and where it does **not** fit — are `eve-patterns.md` §13.
