...
>
> **Same-day follow-up, eve@0.31.3 → 0.33.2** (eve shipped 4 releases within hours of the pass above — 0.32.0, 0.33.0, 0.33.1, 0.33.2). Pulled the CHANGELOG bundled in the package rather than re-reading the docs site. One real break found and fixed: **`defineDynamic` for the model field dropped `fallback` in 0.33.0** — `docs/guides/dynamic-capabilities.md` now states `defineDynamic({ events })` only, "does not compile a default model or model metadata"; every resolver must return a concrete model. Fixed in `eve-concepts.md`, `eve-conventions.md`, `SKILL.md`. Also applied: `ctx.getSandbox().stop()` (0.32.0, custom backends must implement it), channel `turnPolicy` default flipped to `"steer"` (0.33.0), `web_search` now defaults to **Exa** not Parallel (0.32.0), new `eve set --model --reasoning` CLI command (0.33.0). Tool-approval responses renamed `deny`→`cancel` (0.32.0) but we never spelled out the reject keyword, so nothing broke there.
>
> **Second pass, the four files skipped the first time** (`eve-evals.md`, `eve-web-integration.md`, `eve-patterns.md`, `ai-elements.md`), against eve@0.33.2's `.d.ts` files directly rather than the docs prose. `eve-patterns.md` and `ai-elements.md` held up — every subpath, export, and the `ApprovalStatus` union (both bare-string and `{ type, reason? }` object forms are valid, so the two files' differing examples are both correct) checked out. `eve-web-integration.md` had four real problems: the exported type is **`ClientSessionState`**, not bare `SessionState` (wrong in four places); the stream-event type is **`MessageStreamEvent`**, not `HandleMessageStreamEvent`; the `useEveAgent()` return-shape table omitted **`respond()`**, a real returned helper distinct from `send`; and **`maxReconnectAttempts` is not a real option at all** — invented, removed. Also missing entirely: `withEve({ agents })` / `useEveAgent({ agent })` multi-agent mounting, added. `eve-evals.md` was missing the real `--exclude-tag` CLI flag and the `.eve/evals/<timestamp>/` artifacts directory — added, nothing was wrong, just incomplete.
