...
- **`turnPolicy` default changed in 0.33.0** — channel message sends now default to **`"steer"`**: an accepted message cancels and replaces the active turn (steering), instead of the previous wait-for-completion behavior. Set `turnPolicy: "queue"` on a channel, or per `send()`, to keep the old wait-and-queue behavior. This changes what "a message arrives mid-turn" *does* by default — re-check any channel that relied on messages queuing behind an in-flight turn.
