...
**Enforce it at the connection, not in the prompt.** An MCP server exposes whatever it exposes; narrow it where you declare it. The SRE agent pins Datadog to 19 named operations with `?toolsets=core,software-delivery` on the MCP URL, and GitHub to a read-only `repo-explorer` preset with gist operations excluded — so "may look, may not touch" is a property of the tool list the model can even see, rather than an instruction it can be argued out of.
...
> Recipe #7 is distilled from Vercel Labs' MIT template **[marketing-team-eve-template](https://github.com/vercel-labs/marketing-team-eve-template)** (a lead + 5 specialists, 20 agent skills in `SKILL.md` + `references/` form). `[VERIFY]` identifiers against your installed eve — **the template tracks `eve ^0.27.6`, eighteen minors behind 0.45.0**, so treat its API surface as the least current thing in this file even though the *architecture* is what we took from it.
...
## 9. Investigation — hypotheses, evidence, and the finding you may not delete

Distilled from Vercel's [eve incident/SRE agent guide](https://vercel.com/kb/guide/eve-incident-sre-agent): a Slack-resident agent that answers a production alert by pulling context from Datadog, Vercel, GitHub and Slack, then **testing two to four root-cause hypotheses** and reporting in-thread. It is a third shape next to #7 (a team) and #8 (a pipeline): an agent that **concludes** rather than builds.

**a. Hypotheses first, evidence second — not the reverse.** The loop is: load the procedure, gather *only* the context it needs, generate **two to four plausible root causes**, then test each against evidence. An agent that gathers first and reasons after writes a plausible story around whatever it happened to fetch. Two to four is the useful range: one is a guess dressed as a conclusion, ten is a list nobody reads.

**b. Every finding carries its source.** An `evidence_record` tool stores normalized `(finding, sourceUrl)` pairs, deduplicates matches and hands back stable ids (`evidence-1`). A conclusion is then *assembled from* citations rather than asserted and footnoted afterwards.

**c. The rule that makes the rest honest: a finding may not be deleted because it inconveniences a hypothesis.** The agent can add evidence and can abandon a hypothesis; it cannot tidy away the fact that contradicted it. Without this, an agent under pressure to conclude will quietly converge — and its report will look *better* than an honest one, because nothing in it disagrees. This is the same discipline `shadscan` needs (findings are hypotheses until read in context) and `spec-review` enforces by refusing to merge its two axes.

**d. Read-only by default, and the allowlist is per connection.** Both integrations are read-only tokens, and the MCP surface is narrowed at the connection: Datadog to 19 named operations via `?toolsets=core,software-delivery`, GitHub to a `repo-explorer` preset with gist operations excluded. This is #6's read-vs-egress boundary made concrete — the place to enforce "may look, may not touch" is the connection's tool list, not the prompt.

**e. Machine triggers need their own channel.** Alongside Slack, a **webhook channel** (`agent/channels/webhook.ts`) exposes `POST /v1/investigate` for incident tooling: it verifies a shared secret (`x-sre-webhook-secret`, or `Authorization: Bearer` — header wins) and starts the session under `waitUntil` so the HTTP call returns immediately. Don't route machine traffic through a human channel: the auth is different (a secret, not a person), the reply is different (nobody is reading), and the caller wants an ack, not a turn.
