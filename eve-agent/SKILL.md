...
3. Pin the model in `agent/agent.ts` (the eve scaffold default is `anthropic/claude-sonnet-5` via the Vercel AI Gateway; the model can also be `defineDynamic` with a `fallback`) and write a real `agent/instructions.md`. Document the model choice in a comment. For an OpenAI/Gemini model you can also opt into an AI Gateway **service tier** (`priority`/`flex`/`default`) to tune latency vs. cost — see `references/eve-scaffold.md` §3.
