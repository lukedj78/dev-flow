...
The suite is ~260 markdown files written over months. Words drift. This is the **ubiquitous language**: use these terms exactly, and avoid the listed alternatives, so a phrase means the same thing in every skill.
...
**Skill**
One directory at the repo root containing a `SKILL.md`. The thing a coding agent loads.
_Avoid_: command, plugin (a **plugin** is the packaging of them all — see below), prompt, agent.
...
An **eve agent skill** is a markdown file under `agent/skills/` inside a *user's* eve agent — procedural knowledge the model loads at runtime with `load_skill`. It is **not** one of this repo's skills. Write "an eve agent skill" in full whenever both could be meant.
...
**Plugin**
The whole suite packaged for Claude Code (`.claude-plugin/plugin.json`), installed from the marketplace. Singular — there is one plugin containing them all.
