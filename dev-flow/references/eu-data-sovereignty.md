...
| TypeSafe AI Jev (evaluation model, via AI Gateway) | used by eve `auto` (renamed from `autoModel` in eve 0.60.0) and `experimental_evaluate`; since 2026-09-21 also the Gateway's HTTP API (`POST /v1/evaluate`) or an existing TypeSafe client, so a non-TypeScript service reaches the same model | **US** — TypeSafe AI, Inc. (San Francisco, founded 2026); its privacy policy: *"The Services are hosted in the United States"*, with no mention of GDPR, a DPA, SCCs or the DPF and no company address (read 2026-09-17). Reached through AI Gateway the contract is Vercel's. The Gateway lists one endpoint for it, TypeSafe's own, with no region, and our function's region does not move it (2026-09-22). It is a US transfer **when the `state` holds personal data**, so record it and minimise the state; keep identity documents and special-category data out. An EU pin (`inferenceRegion: { scope: 'zone', geoRegion: 'eu' }`) should fail with HTTP 400 — not yet verified by a call. **Self-hostable in the EU:** Rizzo Flow (Apache-2.0, open weights, **TypeSafe-compatible API** — one base URL changes), for text that must not leave the perimeter. It is not free: Vercel has no GPU, so it is a separate always-on machine (Scaleway `L4-1-24G` ≈ €575/month, 2026-09-26), and a cloud GPU host is itself a sub-processor — only on-prem removes the row. Measured numbers and limits: `eve-agent/references/eve-patterns.md` §13 | Gateway catalog marks it `zdr: all`, `no_training: all`; the policy: *"We will not train or fine tune any artificial intelligence or machine learning models on your prompts or other Input"*; input retention not stated | [privacy policy](https://typesafe.ai/legal/privacy-policy) · [Gateway evaluation](https://vercel.com/docs/ai-gateway/modalities/evaluation) |
