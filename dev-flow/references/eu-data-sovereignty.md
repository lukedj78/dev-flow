...
| TypeSafe AI Jev (evaluation model, via AI Gateway) | used by eve `autoModel` and `experimental_evaluate` | EU processing **not verified** — the Gateway evaluation page documents no region option for evaluation; the evaluator receives recent conversation text | the Gateway changelog (2026-09-16) states Zero Data Retention and No Training options apply | [changelog](https://vercel.com/changelog/typesafe-ai-jev-now-available-on-ai-gateway) · [evaluation](https://vercel.com/docs/ai-gateway/modalities/evaluation) |
