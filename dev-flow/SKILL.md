...
| `feature_complete` | **Propose the pre-deploy gates**: `compliance-audit` (GDPR/AI-Act, any stack) + `vercel-doctor` (Vercel cost/perf, web on Vercel). Then deploy: mobile (`expo-rn`) → `rn-eas-deploy`; web → `setup-deploy`; agent → `eve deploy`. |
| `deployed` | Maintenance loop: mobile → more screens via `rn-add-screen`, OTA hotfixes via `rn-eas-build-submit-update`; re-run `compliance-audit` + `vercel-doctor` after material changes; telemetry monitoring. |
