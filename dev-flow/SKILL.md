...
| `feature_complete` | **Propose the pre-deploy gates**: `compliance-audit` (GDPR/AI-Act, any stack) + `vercel-doctor` (Vercel cost/perf, web on Vercel). Then deploy: mobile (`expo-rn`) → `rn-eas-deploy`; web → `vercel-deploy`; agent → `eve deploy`. |
