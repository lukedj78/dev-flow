...
| `feature_complete` | **Propose the three pre-deploy gates**: `compliance-audit` (legal — GDPR/AI-Act, any stack) + `vercel-doctor` (cost/perf — web on Vercel) + `shadscan` (UI quality + a11y — web on shadcn). Then deploy: mobile (`expo-rn`) → `rn-eas-deploy`; web → `setup-deploy`; agent → `eve deploy`. |
