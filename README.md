...
>
> **Golden rules** (enforced on every project, see the contract): **① code is written in English** (identifiers, constants, comments — independent of the conversation language); **② every frontend ships i18n from day one** — web via [next-intl](https://next-intl.dev/), mobile via the RN i18n stack, minimum locales **English + Italian**, no hardcoded user-facing copy; **③ UI is composed only from the chosen library's primitives** — for shadcn the components the CLI installs into `components/ui/` (all of them, at `scaffolded`), changed only through DESIGN.md tokens and `cva` variants; never a hand-rolled primitive, never a second component library, never a forked primitive, and a truly new one only as a recorded exception. Enforced by the design-system lint (undeclared component libraries and the primitives' headless bases are banned imports outside `components/ui/`), `spec-review`'s Standards axis and `shadscan`'s routing.
...
Every skill arrives namespaced (`dev-flow:forms`, `dev-flow:rn-bootstrap`, …), and `/plugin marketplace update` pulls new releases. The shipped set is generated from the canonical taxonomy, so what you install always matches [`skills.json`](./skills.json) and the [CHANGELOG](./CHANGELOG.md).
...
dev-flow is MIT and works offline. It is also allowed to **say that
...
What a dependency buys is *not drifting*, and the toolchain buys it instead. CI runs 29 tests on Node
...
- 🤖 **[Full walkthrough](./docs/example-full-walkthrough.md)** — one product ("Helmsman" AI support desk) exercising every skill, phase by phase: core → design → monorepo → web → mobile → agent (eve) → voice/realtime → deploy.
...
- 🧠 **[Obsidian](./docs/OBSIDIAN.md)** — this repo is also an Obsidian vault (config committed): graph view over skills → references, backlinks, full-text search across every skill.
...
```
<skill-name>/                one folder per skill, FLAT at the root
...
**Input**: a dev-flow project whose API is callable by something other than its own frontend — route handlers under `app/api/`, an OpenAPI spec, a public eve agent, an MCP server — plus a deployed origin.
**Output**: `skills/<slug>/SKILL.md` **in the product's repo**, plus the README row and the `npx skills add owner/repo` line for the product's users. Records `meta.json#stack.agent_skill`; no phase bump.

**Why**: an API only its own frontend calls is an implementation detail. One a coding agent could drive is a distribution channel, and it needs exactly one artefact to become usable — a runbook telling that agent how the product works. [`sleekdotdesign/agent-skills`](https://github.com/sleekdotdesign/agent-skills) is the proof: a design service published one 563-line SKILL.md and became drivable from Claude Code, Cursor and Codex without shipping an SDK for any of them. `references/anatomy.md` is what that file taught us, section by section — device flow before raw keys, the loop before the endpoint catalogue, the price stated before any step that leads to a payment page, and a common-mistakes table that can only be written by having driven the thing.

⚠️ **The output belongs to the product, not to this repo.** It ships and versions with that product and is aimed at whoever installs *it*. It is never added to dev-flow's own skills, never copied into `~/.claude/skills/`, never listed in `install.sh` — the same relationship dev-flow has with the `app/` it builds and does not own. `dev-flow` raises it once, at `feature_complete` → `deployed`, and only when the surface is genuinely public: a skill describing an internal route handler is a lie with a nice table in it.
...
npm facts are read with the history a name hides. A name that **changed hands** — two years of silence, then a different repository and no maintainer in common — is D8, and its age (D5) counts from the new owner's first release. `cn` 0.1.x was a 2013 joke package and `motion` 5.0-beta was another project; both are now what our stack uses, checked and recorded. Under 1.0 a caret locks the minor, so `cn@^0.2.4` does not accept 0.3.x (D7).

**The agent always stops and asks before `allow` and `approve`**: it runs the read-only review, shows the report, and waits for a yes to the registry and a yes to the item, whatever the tier and however hurried the prompt. `--by` names the person who said yes, never the agent or the OS login. This was measured, not assumed. With a looser wording, [`evals/compliance`](./evals/compliance/README.md) caught the agent allowlisting and approving itself in two sessions out of three, writing the operator's login into `--by`, so the lock recorded an approval nobody gave. The hook now answers `"ask"` on both commands, so the user gets a permission prompt even when the agent skipped the question.

Four barriers, each mechanical: an **allowlist** (a new registry is the user's decision, with a reason); a **review** of the item's whole closure — config/tooling targets, host process execution, obfuscation, env vars, theme tokens (DESIGN.md owns them), npm existence, licence, install scripts and age, and for eve code the tool keys the *installed* eve accepts, ungated side effects, global credentials, a shared store, US-shaped PII regexes; a **snapshot** installed with `shadcn@4.21.0` instead of the URL (local items and root-relative `registryDependencies` verified on that version); and **nothing around it** — the hook denies every other `shadcn add @…`, URL install, `eve add @…` and `eve registry add` using the documented `permissionDecision: "deny"` contract, while `check` catches edited snapshots, stray vendor files, unlisted registries in any `components.json`, and a design-lint `--max-warnings` raised to make room for imported code. Blocking findings are fixed, ported by hand (`eve-registry-porting`), or accepted one by one with a written reason — never waved through. `@coss` is the one registry trusted live, and only when `stack.ui = "coss"`. The rules are heuristics over source text, stated as such: typecheck, the capped lint, `eve build` and tests still run after install. 30 tests, no network, in CI.
...
**How it works**: scores the codebase against a fixed **10-point risk register** (R1 DSAR · R2 consent/cookies · R3 EU data residency & transfers · R4 retention/TTL & PII-scrubbing · R5 AI-transparency Art. 50 · R6 high-risk Annex III/DPIA · R7 PII in logs · R8 sub-processors · R9 special-category data · R10 memory/manipulation). `scripts/scan.py` is a fast first-pass signal; every hit is verified in code before it lands in the report. **R4 has one dimension a code read cannot reach**: scrubbing that exists and silently does not work. Where the scrubber is a *model* rather than a regex it can return zero findings on a document full of them and answer `200` — so the audit asks what the model was trained on versus what the product ingests, whether the checked rules outrank it, whether a zero-finding result is surfaced rather than read as a clean document, and whether a token limit is being crossed in silence. **Auto-applies only the safe, mechanical fixes and flags every decision** (which EU region, legal basis, whether high-risk) — it never decides law for you. Horizontal capability: run it any time; dev-flow proposes it as a **pre-deploy gate** at `feature_complete` and re-runs it in the `deployed` maintenance loop. Records `meta.json#compliance`, never bumps `phase`. **Not legal advice** — it produces engineering findings + a DPIA template; a DPO/counsel confirms.

**The decision it audits is taken earlier.** Where the data lives is decided at the stack decision, not discovered at the audit: `dev-flow` asks four questions (EU data subjects, sensitive categories, sector residency, CLOUD Act) and records `stack.data_residency` = `eu` · `eu-sovereign` · `none` with [`dev-flow/scripts/data_residency.py`](./dev-flow/scripts/data_residency.py); every `module-add` / `rn-module-add` sets the provider's EU region and adds it to one register (`meta.json#compliance.sub_processors` → `docs/compliance/subprocessors.md`); this skill then checks the code against that record. A provider with no EU option is **flagged with its transfer basis and never blocks** — the phase gate only refuses an undecided residency. The how-to is [`dev-flow/references/eu-data-sovereignty.md`](./dev-flow/references/eu-data-sovereignty.md): a table of 13 service categories with the default, its EU setting and European alternatives, each row sourced on the vendor's own page (2026-09-15), and the regulations with primary links and a re-verification cadence — including the AI Act as amended by Regulation (EU) 2026/1744 (Annex III high-risk from 2 Dec 2027, Art. 50 from 2 Aug 2026) and the pending DPF appeal C-703/25 P. Found on the way: Anthropic's own API has no EU inference, Resend's EU region moves sending but not storage, Firebase Auth runs only in the US.
...
- **Node graphs** via [React Flow](https://reactflow.dev) (`references/react-flow.md`), written after finding it in five of our projects wired five ways. Only for a graph the user acts on — a read-only diagram that fits is cards. The details that were wrong in practice: theme through the **un-suffixed** `--xy-*` variables (the `-default` ones are redefined under `.react-flow.dark` and beat a token mapping in dark mode), `colorMode` from `next-themes`' `resolvedTheme` rather than `"system"` or a constant, a list instead of a canvas below `md:`, `ariaLabelConfig` fed from next-intl, React Flow UI's shadcn nodes installed through `registry-intake`, and the attribution kept — the installed types ask to hide it only with a subscription, and all six of our flows hid it without recording one.
...
- **The type scale lives inside shadcn's own scale** — each DESIGN.md level redefines its nearest Tailwind slot (`--text-xs`, `--text-sm`, `--text-base`, `--text-xl`, `--text-3xl`…) with its line-height, tracking and weight, and **never** a parallel set of semantic names. The primitives are written with `text-sm` ×92, `text-xs` ×29 and `text-base` ×10 and speak no other vocabulary, so a `text-body` beside them compiles and reaches no `<Button>`. This exists because a real project shipped **zero type tokens and 531 `text-[…]` literals**; moving it into the slots changed exactly two sizes across 680 measured text nodes, both intended, and gave the 131 primitive usages DESIGN.md's line-height for free.
- **A design-system lint from day one, wired by a script and enforced by the phase gate** — [`@shadcn/lint`](https://github.com/shadcn-ui/lint), installed and wired by `scripts/setup_design_lint.py` for all three web topologies (single app, web+mobile, shadcn monorepo) and checked by `dev-flow`'s `set-phase`, which refuses `scaffolded` until it is wired or an opt-out is recorded with its reason (`references/design-system-lint.md`). What earns it the place is the error: *"`rounded-full` is not allowed on `<Button>`: `<Button>` owns its shape. Use a variant…"* names the fix, so an agent corrects in one round. The config in the reference was run **verbatim** against the 61 primitives of a real scaffold: 0 errors from the plugin, every override holding. Two traps are written down because they were hit: a custom `--shadow-*` token is reported as an undeclared colour, and the fix must allow it **by exact name** — `shadow-*` was verified to let `shadow-pink-500` through; and a flood of unknown-class warnings means a theme `@import` did not resolve, which the linter says in one line before the flood. A third came from upstream. Since September 2026 shadcn's registry imports `cn` from the [`cn`](https://github.com/shadcn-ui/cn) package, and in a project whose `lib/utils` teaches the merger its own type scale that stock instance silently drops classes. There, and only there, the preset bans the package import, primitives included. Elsewhere the import is left alone, because it is the same function.
...
**Model choice** stays a single pinned model by default. Since eve@0.58.1 an agent can instead take **`auto`** (experimental; named `autoModel` before eve 0.60.0 renamed it and moved it from `eve/experimental/evaluate` to `eve/models`): an AI Gateway evaluation model (TypeSafe AI's Jev) picks per turn from a described allowlist — at the cost of one evaluation call per turn, and with the evaluator becoming a processor that sees recent conversation text, which the data-residency register has to record. The same evaluation models are worth more inside the agent than in front of it: `references/eve-patterns.md` §13 uses their typed answers as a risk score in an `approval` policy, a guardrail inside a tool (hooks are observe-only, so never there), a refusal check graded with `t.check` in evals, and a pre-filter over a queue. It gates on probability *and* the separate confidence Jev returns for choices and scores, unit-tests those thresholds against `ai/test`'s mock evaluation model, fails closed, is calibrated with evals, and is never the only control on something irreversible. Since 2026-09-21 the Gateway also serves Jev over a plain HTTP API and to an existing TypeSafe client, so a Python or Go service asks the same typed questions without an SDK. Jev runs only on TypeSafe's US-hosted endpoint, wherever our own code runs, so personal data stays out of its `state`. Where nothing may leave the perimeter — a firm's filings, special-category data — §13 names the self-hostable alternative: **Rizzo Flow** (Apache-2.0, open weights) whose HTTP API speaks the TypeSafe shape, so the switch is one base URL. With the arithmetic that decides it: an EU GPU runs ≈ €575/month against $0.00013 per document hosted, so self-hosting buys confidentiality, never savings — and Vercel has no GPU, so it is always a separate service. Their own agent skill ships with it; we point at it instead of rewriting it.
...
**Thirteen composed recipes** in `references/eve-patterns.md`, distilled from the docs, from three MIT Vercel Labs templates and from Vercel's own SRE-agent guide. The first four are the multi-tenant backbone (tenant auth · per-tenant approvals · tenant-scoped memory · dynamic scheduling); then traceability and the read-vs-egress boundary; then **three shapes an agent can take** — §7 a **team** (a lead routing to non-overlapping specialists), §8 a **pipeline** (declared stations, and what a run with *nobody watching* may do), §9 an **investigation** (an agent that concludes rather than builds); §10 the **cross-channel notification**, where `to()` is a handoff and a notification wants the platform API plus an app-owned outbox; §11 **untrusted content**, fenced and gated; and §12 **backlog triage**, where the second agent argues the opposite and only runs above a confidence threshold.

§6 carries a third answer for an egress a tool cannot make safely, next to *fail closed* and *ask permission*: **redact locally and send the placeholders**, restoring the real values from the reply. It comes with the trap that makes it worse than fail-closed when it goes wrong — fail-closed at least stops, while a broken redactor ships the document and reports that it was cleaned. Measured, not hypothetical: a BERT fine-tuned on Wikipedia prose found **zero** person names across a 325-page bank statement, and the endpoint answered `200`.
...
**Between View Transitions and Motion there is a seam, and it now has two doors.** Motion's imperative `animateView(update)` and, since `motion@13.4.0`, the declarative `<AnimateView>` (`motion/react-animate-view`) both choreograph the browser's View Transitions instead of replacing them — cheaper than Motion layout animation, richer than a bare `document.startViewTransition`. The component carries a trap worth knowing before anyone installs it: it renders React's `<ViewTransition>` and its source throws *"AnimateView requires React 19.3 or later."*, while `motion`'s peer range still accepts React 18 and 19 — so npm installs it on a React 19.2 project and it breaks in the browser, not at install. `ViewTransition` itself graduated to a stable export in `react@19.3.0`; our projects are on 19.2.4, so the reference marks `<AnimateView>` as the next rung, not today's. Like vgpu, its docs never mention `prefers-reduced-motion`: that path stays ours.
...
### `iframe-canvas` — how many live previews can this canvas actually afford?

**Input**: "infinite canvas" / "canvas infinita" / "tavolozza tipo Figma" / "artboard" / "live preview in an iframe" / "visual editor", or a preview iframe that went slow, blurry or stopped receiving clicks.
**Output**: the architecture named out loud and recorded in `stack.canvas` — all-live, snapshot-and-hydrate, single-live, or rendered — plus the camera, the isolation choice and the `postMessage` contract that follow from it.

**How it works**: it starts by refusing the premise. Figma's canvas is not DOM — Evan Wallace's account says HTML and SVG were rejected for *"DOM access overhead"* and because browsers optimise for scrolling, not zooming — so "like Figma" imports constraints Figma paid a rewrite to escape. The family this skill serves is the AI builders (Reweb, Sleek, Plasmic, Builder.io, Onlook), where the scarce resource is **browsing contexts**, not draw calls: same-site iframes share the parent's renderer process and main thread, and the tab has ~2 GB.

The load-bearing precedent is that **Plasmic replaced its multi-artboard view with a single-artboard editing mode**, citing performance, and kept multi-artboard only as a side-by-side comparison mode. So the default here is one live artboard and many static ones, not the reverse. The rest is the part that gets built wrong once and stays wrong: a camera owning both coordinate spaces (with `1 / scale` on anything that must stay 1px), overlays drawn in the parent rather than injected into the frame, one declared interaction mode instead of inline pointer gating, and a protocol written as if it were cross-origin even when it isn't — because that is the switch you will want the day the canvas slows down or the content stops being yours.

Every claim traces to [`references/production-survey.md`](./iframe-canvas/references/production-survey.md), which also says what the survey lacks: nobody publishes the numbers at which one of these falls over.
...
1. **`python3 scripts/lint_skills.py`** — 19 checks. The description cap in particular: over 1024
...
```bash
# Sanity-check every skill — 19 checks (frontmatter YAML + the 1024-char
...
# Regenerate skills.json (the machine-readable registry of every skill)
...
**Most of the checks exist because the same thing kept happening: the content held and the metadata rotted.** Check 8 catches a capability documented in a skill body but missing from its `description` — the skill would never load for the request that needs it. Check 10 catches a skill that exists but is absent from the README catalogue. Check 11 catches a skill count stated in prose that no longer matches reality — it found six on its first run, four of them in phrasings that a grep for the obvious form ("N skills") never matched — a bare count after "There are", one inside "packaging of all N", and both installers' header comments. It has since gained a pattern for every phrasing that slipped past it — the skill map's metric card (a bare value with no adjacent word), the plugin blurb, the docs link, and a fake terminal line where the em-dash the pattern anchored on belonged to a different number three words earlier. The durable fix was the other direction: the count is now absent from the prose that does not need it. **Check 12** compares what is installed against what is here, and reports rather than fails: a divergence is a fact about your machine, not about the commit. **Check 13** compares the skill map's per-skill meta — the bare `1122·9r·2s` beside each name — against `skills.json`; nothing writes those numbers and no prose pattern can reach them, and on its first run **9 of the 15 rows were wrong**. **Check 14** guards the count in this very sentence, because the commit that added checks 13 and 14 first wrote "13" in three places. **Check 15** compares a file a skill hands to a project against the copy CI tested — `guards.template.ts` against `agent-guards/src/guards.ts` — because a drift there ships untested code to every new project while the suite stays green. **Check 16** derives the skill map's metric cards from `skills.json` and the contract's `Phase` enum: two of them had drifted so far — `web · next 16` and `how-tos` both reading numbers no family or role in the registry produced — that their definition was no longer recoverable from the page, only from their siblings.
