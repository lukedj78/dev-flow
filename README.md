...
**Eight composed recipes** in `references/eve-patterns.md`, distilled from the docs and from three MIT Vercel Labs templates. The first four are the multi-tenant backbone (tenant auth · per-tenant approvals · tenant-scoped memory · dynamic scheduling); then traceability, the read-vs-egress boundary, the **multi-agent team** (a lead routing to non-overlapping specialists), and **§8 the autonomous pipeline** — declared stations, and what a run with *nobody watching* may do.
