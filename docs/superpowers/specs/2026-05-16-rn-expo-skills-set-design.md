...
├── rn-backend/                  │  (provider-agnostic: Supabase / Firebase / custom REST / tRPC)
...
| K8 | `rn-backend` | **Provider-agnostic** backend integration: auth flow (email/oauth/magic link), token storage (`expo-secure-store`), client-side auth state, server-side data access (RLS-style policies vs custom API auth), realtime patterns, file storage. Sub-references per provider: `references/supabase.md` (default), `references/firebase.md`, `references/custom-rest.md`, `references/trpc.md`. Choose at bootstrap or per-feature; the skill enforces shared patterns (secure token handling, query-key conventions, auth-gate routing) regardless of provider. | lean | K1, K5 |
...
| K8 `rn-backend` | 🆓 — | 💰 "Backend Basics" + "Supabase" | 📘 supabase.com/docs, 📘 firebase.google.com/docs/auth, 📘 trpc.io/docs, 📘 expo-secure-store, 📘 expo-auth-session | ⚙️ Supabase is the *default* (matches the course); Firebase, custom REST, and tRPC are first-class alternatives via sub-references. The skill enforces shared patterns: secure-store for tokens, never AsyncStorage; auth state propagated via Zustand store + TanStack Query; auth-gate routing via Expo Router `(app)/_layout.tsx`. |
...
10. `rn-backend` (K8) — provider-agnostic (Supabase default + Firebase / custom REST / tRPC)
...
## 12. Decisioni di revisione del design (post-Wave 2)

### K8 `rn-backend` — provider-agnostic invece di Supabase-only (2026-05-22)

**Cambiamento**: K8 era originariamente `rn-backend-supabase`, lockata sul provider Supabase. Rinominata in `rn-backend` con architettura "core + sub-references":

- **Core (skill body)**: pattern shared cross-provider — secure token storage (`expo-secure-store`, MAI AsyncStorage per token), auth state via Zustand + TanStack Query, auth-gate routing via Expo Router `(app)/_layout.tsx`, query keys per dati auth, refetch on AppState change.
- **Sub-references per provider**:
  - `references/supabase.md` — il default (corso codewithbeto), `@supabase/supabase-js`, RLS, realtime, edge functions, tipi via `supabase gen types`.
  - `references/firebase.md` — alternativa BaaS managed, `firebase` SDK, Firestore vs Realtime Database, FCM, vendor lock-in più stretto.
  - `references/custom-rest.md` — il tuo backend (Node/Rails/Django/…), JWT in secure-store, refresh token flow, retry-on-401.
  - `references/trpc.md` — più tipato, `@trpc/client` + integrazione TanStack Query, schema condiviso server↔client.
- **Decision tree**: `references/decision-tree.md` — quando Supabase, quando Firebase, quando custom REST, quando tRPC.

**Razionale**: legare K8 a un provider specifico significherebbe rifare la skill per ogni progetto con un backend diverso. Il valore della skill è insegnare i *pattern condivisi* (token storage sicuro, auth state, RLS-style policies vs API auth), non la sintassi di un SDK. Il provider è una scelta di runtime; la skill resta unica.

**Default mantenuto**: Supabase resta la raccomandazione di default per coerenza col corso e con `rn-bootstrap` che ne usa il preset. Cambia se l'utente sceglie altro in fase PRD / bootstrap.

**Implicazione su `rn-module-add` (O3)**: anche lui resta agnostico — i sotto-moduli `auth` / `db` / `storage` / `realtime` leggono `meta.json#stack.auth` (e simili) per sapere quale provider è stato scelto e generano il wiring corretto.
