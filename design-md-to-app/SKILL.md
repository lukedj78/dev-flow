...
- shadcn → `references/shadcn-mapping.md`. **Monorepo first:** if `meta.json#stack.framework == "monorepo"` and the monorepo is web-centric (no NativeWind/mobile side — e.g. web-only or web + agent), follow that reference's **"Monorepo (shared `packages/ui`)"** section: scaffold with `shadcn init --monorepo` so primitives land in a shared `packages/ui` (`@workspace/ui`), NOT in `apps/web/components/ui/`. The single-app flow below (and `--no-monorepo`) is for non-monorepo projects (or the web+mobile case where components stay app-local). **Two visual-config paths — they are mutually exclusive:**
  ...
  - **Pass the create parameters from `meta.json#stack`** (shadcn CLI v4): `--base ${stack.ui_base ?? "base"}` (Base UI / Radix / React Aria primitives), `${stack.css_variables === false ? "--no-css-variables" : "--css-variables"}`, and `--rtl` when `stack.rtl`. **There is no `--base-color` flag** — see `shadcn-mapping.md` § About the `init` and `add` commands; `stack.base_color` is never passed to the CLI, it's applied purely through the DESIGN.md token override (`registry.json`) in a later step, same as theme and fonts. **`--base` is NOT overridden by DESIGN.md** — it picks the primitive engine, so honor `stack.ui_base` exactly. If `stack.ui_base` is unset, ask before scaffolding (don't silently pick a base on a fresh project).
