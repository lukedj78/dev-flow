...
- "shadcn" → `references/shadcn-mapping.md` (+ `stack.ui_base` = base|radix|aria)
...
| **Node graph / pipeline editor / workflow builder / org chart the user acts on** | **React Flow** (`@xyflow/react`) — custom nodes composed from shadcn primitives; node UI from React Flow UI through `registry-intake` | Pan/zoom, drag-to-connect, keyboard focus, `ariaLabelConfig` for i18n. A read-only graph that fits the screen is cards, not a canvas. Theme through the un-suffixed `--xy-*` variables, `colorMode` from `resolvedTheme`, a list below `md:`, attribution kept unless subscribed. See `references/react-flow.md`; record `stack.graphs = "react-flow"` |
...
| Toast / notification | `Toast` (shadcn's first-party Toast, Base UI, 2026-07 — actions, status types, promises, stacking, swipe-dismiss; `sonner` still fine on Radix) | ARIA live region, stacking, swipe |
...
- **Next 16.3 (stable, 2026-08-03) — scaffold on `next@latest`.** Most of 16.3 is zero-config (dev memory, build cache, native Node streams, TS 7 type-checking) and needs no decision. **Instant Navigations is opt-in**: `cacheComponents: true` + `partialPrefetching: true`. For a *new* project it's a reasonable default to offer — ask once, and if the user says yes, remember that Cache Components turns on navigation validation in dev (`validationLevel: 'warning'`), which is a feature, not noise. Don't switch an *existing* app silently: route to the official [migration guide](https://nextjs.org/docs/app/guides/migrating-to-cache-components). Details in `data-fetching/SKILL.md` §Instant Navigations.
